...
library(fpp3)
library(GGally)
rm(list = ls()) # delete all objects from the current R environment/workspace.
...
#! plot feature vs time for each feature
ncs %>% 
  pivot_longer(-Quarter) %>% # pivot first before plot 
...
#! create forcasted for each predictor
DPIPC.base <- ncs %>% 
  model(SNAIVE(DPIPC)) %>% 
  forecast(h=4) %>% 
  pull(.mean)
...
#! get forecasted value from forecasted predictor
...
#! TS cross validation 
trCV <- ncs_long %>% # need to convert to long first 
  slice(1:(n()-4*4)) %>% 
